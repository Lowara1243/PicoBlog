...
PicoBlog is configured using environment variables. You can set these in a `.env` file in the project root.

| Variable | Description | Default |
| :--- | :--- | :--- |
| `SECRET_KEY` | **Required.** A long random string used for session security. | *None* (Must be set in production) |
| `FLASK_ENV` | Set to `production` for production environments. | `development` |
| `DATABASE_URL` | Database connection URL. | `sqlite:////path/to/app.db` |
| `REQUIRE_LOGIN` | `True` to require login for all posts (private mode). `False` allows public viewing of untagged posts. | `True` |
