...
*   **CI/CD:** Автоматическое тестирование и конвейеры развертывания.
...
Самый простой способ запустить PicoBlog — использовать Docker.
...
2.  **Запуск:**
    ...
    ```
    Блог будет доступен по адресу `http://localhost:8000`.
...
*   **uv:** Быстрый установщик пакетов Python.
    ...
    pip install uv
    ```
*   **Node.js 16+ & npm:** Требуется для сборки Tailwind CSS.
    ```bash
    # Debian/Ubuntu
...
PicoBlog настраивается с помощью переменных окружения. Вы можете установить их в файле `.env` в корне проекта.

| Переменная | Описание | По умолчанию |
| :--- | :--- | :--- |
| `SECRET_KEY` | **Обязательно.** Длинная случайная строка для безопасности сессий. | *Нет* (Должно быть установлено в продакшене) |
| `FLASK_ENV` | Установите `production` для производственной среды. | `development` |
| `DATABASE_URL` | URL подключения к базе данных. | `sqlite:////path/to/app.db` |
| `REQUIRE_LOGIN` | `True` требует вход для всех постов. `False` разрешает публичный просмотр постов без тегов. | `True` |

### 4. Переменные окружения

Создайте файл `.env` в корневом каталоге проекта на основе `.env.example` и укажите ваш секретный ключ и URL базы данных.
